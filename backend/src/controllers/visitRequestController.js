...
function isValidIranMobile(mobile) { return /^09\d{9}$/.test(mobile); }
...
exports.createVisitRequest = async (req, res) => {
    try {
        ...
        if (!car_id || !Number.isFinite(car_id) || car_id <= 0) return res.status(400).json({ error: "خودرو مشخص نشده." });
        let car = null;
        try {
            const simpleCar = await db.query(`SELECT id, brand, model, year, price_aed, status, brand_id, model_id FROM cars WHERE id = $1 LIMIT 1`, [car_id]);
            if (simpleCar.rows.length) {
                ...
                car = { id: c.id, brand_name: brandName, model_name: modelName, brand: c.brand, model: c.model, year: c.year, price_aed: c.price_aed };
            }
        } catch (carErr) {
            console.error("CAR FETCH ERROR:", carErr.message);
            return res.status(500).json({ error: "خطا در بررسی خودرو: " + carErr.message });
        }
        if (!car) return res.status(404).json({ error: "خودرو یافت نشد." });
        ...
        let car_year = null;
        try { const y = Number(car.year); if (Number.isFinite(y) && y > 1900 && y < 2100) car_year = Math.trunc(y); } catch {}
        let car_price_aed = null;
        try { const p = Number(car.price_aed); if (Number.isFinite(p) && p >= 0) car_price_aed = p; } catch {}
        const car_title = `${car_brand} ${car_model} ${car_year || ""}`.trim().slice(0, 250);
        try {
            const dupCheck = await db.query(`SELECT id FROM visit_requests WHERE mobile = $1 AND car_id = $2 AND created_at > NOW() - INTERVAL '5 minutes' LIMIT 1`, [mobile, car_id]);
            if (dupCheck.rows.length) return res.status(429).json({ error: "درخواست شما برای همین خودرو همین الان ثبت شده است." });
        } catch (dupErr) {}
        const ip = String(req.headers["x-forwarded-for"] || req.ip || "").split(",")[0].trim().slice(0, 64);
        let result;
        try {
            result = await db.query(`INSERT INTO visit_requests (first_name, last_name, mobile, car_id, car_brand, car_model, car_year, car_title, car_price_aed, status, sms_consent, sms_consent_at, ip_address) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,'جدید',$10, CASE WHEN $10 = true THEN NOW() ELSE NULL END, $11) RETURNING id, first_name, last_name, mobile, car_id, car_title, status, sms_consent, created_at`, [first_name, last_name, mobile, car_id, car_brand, car_model, car_year, car_title, car_price_aed, sms_consent, ip]);
        } catch (insErr) {
            console.error("INSERT ERROR:", insErr.message);
            return res.status(500).json({ error: "خطا در ذخیره: " + insErr.message });
        }
        ...
        setImmediate(async () => {
            try {
                ...
                if (smsConf.enabled) {
                    await smsService.notifyAdminNewRequest({ firstName: first_name, lastName: last_name, mobile, carTitle: car_title, carId: car_id });
                    await smsService.sendCustomerConfirmation({ mobile, firstName: first_name, carTitle: car_title });
                }
            } catch (smsErr) {
                console.error("SMS background error:", smsErr.message);
            }
        });
        return res.status(201).json({ message: "درخواست شما با موفقیت ثبت شد", request: result.rows[0] });
    } catch (error) {
        console.error("CREATE VISIT REQUEST ERROR:", error.message, error.stack);
...
exports.getVisitRequests = async (req, res) => {
    try {
        ...
        let query = `SELECT vr.*, c.brand AS car_brand_raw, c.model AS car_model_raw FROM visit_requests vr LEFT JOIN cars c ON vr.car_id = c.id WHERE 1=1`;
...
exports.updateVisitRequestStatus = async (req, res) => {
    try {
        ...
        if (!Number.isFinite(id) || id <= 0) return res.status(400).json({ error: "شناسه نامعتبر است." });
        if (!status || !ALLOWED_STATUSES.includes(status)) return res.status(400).json({ error: `وضعیت نامعتبر است.` });
        const result = await db.query(`UPDATE visit_requests SET status = $1, updated_at = NOW() WHERE id = $2 RETURNING *`, [status, id]);
        if (!result.rows.length) return res.status(404).json({ error: "درخواست یافت نشد." });
...
exports.exportVisitRequestsCsv = async (req, res) => {
    try {
        ...
        let where = "";
        if (String(consent).toLowerCase() === "true" || consent === "1") where = "WHERE sms_consent = true";
...
exports.bulkSmsToConsented = async (req, res) => {
    try {
        ...
        const sanitizeName = (s) => String(s || "").trim().slice(0, 40) || "کاربر";
        const sanitizeMsg = (s) => String(s || "").trim().slice(0, 100) || "x";
        const sanitizeDash = (s) => String(s || "").trim().replace(/\s+/g, "-").slice(0, 30) || "x";
        ...
        let bulkTemplate = template || "bulk-greeting";
        const uniqTemplates = [...new Set([bulkTemplate, "bulk-greeting", conf.template].filter(Boolean))];

        const results = [];
        for (const row of rows) {
            const fullName = `${row.first_name || ""} ${row.last_name || ""}`.trim() || "کاربر";
            const fullNameSpace = sanitizeName(fullName); // با فاصله
            const fullNameDash = sanitizeDash(fullName); // با -
            const msgSpace = sanitizeMsg(message || "");
            const msgDash = sanitizeDash(message || "");

            let sent = false;
            let lastErr = "";
            for (const tmpl of uniqTemplates) {
                // اول با فاصله امتحان کن (الگوی جدید تایید شده)
                try {
                    let r = await smsService.lookupViaKavenegar({
                        ...
                        token: fullNameSpace,
                        token2: msgSpace,
                        token10: fullNameSpace,
                        token20: msgSpace
                    });
                    results.push({ mobile: row.mobile, ok: true, method: r.method || "lookup", template: tmpl, withSpace: true });
                    sent = true;
                    break;
                } catch (e) {
                    lastErr = e.message;
                    // اگر 431 داد (فاصله مشکل دارد)، با - امتحان کن
                    if (e.message.includes("431")) {
                        try {
                            ...
                            results.push({ mobile: row.mobile, ok: true, method: r2.method || "lookup", template: tmpl, withSpace: false, note: "فاصله به - تبدیل شد" });
                            sent = true;
                            break;
                        } catch (e2) {
                            lastErr = e2.message;
        ...
        return res.json({ ok: true, total: mobiles.length, sent: results.filter((r)=>r.ok).length, failed: results.filter((r)=>!r.ok).length, results, usedTemplate: uniqTemplates[0] });
    } catch (e) {
