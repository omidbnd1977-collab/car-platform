...
exports.createVisitRequest = async (req, res) => {
    try {
        ...
        console.log(`VISIT REQUEST CREATED: #${result.rows[0].id} ${mobile} -> car ${car_id}`);
        setImmediate(async () => {
            try {
            ...
            } catch (e) { console.error("SMS bg error:", e.message); }
        });
        return res.status(201).json({ message: "درخواست شما با موفقیت ثبت شد", request: result.rows[0] });
    } catch (e) {
        console.error("CREATE VISIT ERROR:", e.message, e.stack);
        return res.status(500).json({ error: "خطا در ثبت درخواست: " + e.message });
    }
};
...
exports.bulkSmsToConsented = async (req, res) => {
    try {
        ...
        if (Array.isArray(selectedMobiles) && selectedMobiles.length > 0) {
            ...
            rows = q.rows;
            const found = new Set(rows.map((r) => r.mobile));
            for (const m of cleaned) if (!found.has(m)) rows.push({ mobile: m, first_name: "کاربر", last_name: "" });
        } else {
